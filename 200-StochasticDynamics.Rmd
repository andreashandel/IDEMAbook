...
The host extinction approach is also often considered and used for vector-borne diseases. Here, the idea is to drive one of the hosts (commonly called the vector) to extinction. The obvious reasoning is that if there are fewer vectors (e.g. mosquitoes), the chances for humans to get infected are lower. Widespread use of DDT or insecticide-coated bed-nets are examples of this. A similar but more controversial approach is the attempted control of rabies in humans by killing its main host, vampire-bats [@stoner-duncan14].


##Disease Emergence
The flip-side of extinction is emergence of a new disease. During extinction, infected/pathogen numbers move from a level that can be decently approximated by a deterministic model to numbers so low that it requires a stochastic analysis approach - with possible eventual extinction. During emergence, the new disease starts at zero, then is introduced at very low numbers (possibly only a single introduction) into a new population, and "bounces around" for a while at low numbers. If conditions are right (i.e. local reproductive number greater than 1), the disease might take off and spread [@antia03]. It might eventually reach high enough numbers that a deterministic approximation which focuses on the mean dynamics is warranted. Any recent emerging disease outbreak followed that pattern, with some ID reaching high enough numbers that a deterministic approach was reasonable (2014 Ebola, 2009 Influenza pandemic), while other diseases only lead to relatively few cases for which the stochastic modeling and analysis approach is generally better (e.g. 2013 SARS, most other small Ebola outbreaks).
...
##Further Resources
* Some further analyses of critical community size and measles can be found in [@bartlett57; @bartlett60; @keeling97; @keeling97a].
