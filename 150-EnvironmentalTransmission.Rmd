...
* Know the hallmarks of indirect transmission
* Understand what indirect transmission implies for control strategies
* Identify diseases for which the indirect transmission pathway is important


##Introduction
While the distinction between direct and indirect transmission is not clear-cut, we usually consider an ID to have an indirect mode of transmission if the time spent outside the main host is _important_ for the whole transmission cycle. The two main types of stages where a pathogen resides outside the main host are the abiotic environment or another host species. In the case of the former, we consider it to be environmental transmission, in the case of the latter, it is vector-borne. Of course, some ID are even more complicated and have both vector and environmental stages. This chapter focuses on environmental transmission.
...
The environmental stage is a potentially great target for control mechanisms. Desinfecting surfaces with chemicals such as bleach is common practice in hospitals and other places where contamination might be common and needs to be minimized. Similarly, attempts have been made to disinfect the air with for instance UV radiation. Unfortunately, such radiation can be harmful to humans, and thus this approach only works within a system that moves and circulates air (e.g. an air-conditioning system with a stage at which the circulated air is irradiated). All modern water treatment plants have systems that remove pathogens - which is why diseases like Cholera are not a problem for countries with good sanitation, but are still a problem in countries where sanitation is less good. Despite good sanitation, other pathogens like norovirus are hard to control and still often cause local outbreaks. One reason for that is that these viruses are hard to kill and many regular cleaning agents do not work.
...
##Summary and Cartoon
This chapter provided an introduction to environmental transmission and its
